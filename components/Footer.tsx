  ...
  return (
    <footer style={{ background: '#1c1c1a', padding: '3.5rem 0' }}>
      <div style={{ maxWidth: '1180px', margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
        <a href="#hero">
          <Image src="/logo-light.png" alt="Not a Croissant™" width={130} height={50} style={{ opacity: 0.85 }} />
        </a>
        <nav style={{ display: 'flex', gap: '2rem' }}>
          {[['#story', 'Story'], ['#process', 'Process'], ['#order', 'Join']].map(([href, label]) => (
            <a key={href} href={href} style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(246,221,190,0.4)', textDecoration: 'none', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#f6ddbe')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(246,221,190,0.4)')}
            >{label}</a>
          ))}
        </nav>
        <p style={{ fontSize: '0.72rem', color: 'rgba(246,221,190,0.3)', letterSpacing: '0.04em' }}>© 2026 Not a Croissant™ · New York</p>
        <a href="https://instagram.com/notacroissantnyc" target="_blank" rel="noopener" style={{ color: 'rgba(246,221,190,0.45)', transition: 'color 0.2s' }}
          onMouseEnter={e => (e.currentTarget.style.color = '#f6ddbe')}
